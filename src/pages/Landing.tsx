...
export default function Landing() {
  ...
  const S = {
    btn: {
      ...
      color: '#0C1825', padding: '16px 32px', borderRadius: 12,
      fontSize: 17, fontWeight: 700, textDecoration: 'none',
    } as React.CSSProperties,
    card: {
      background: '#112236', border: '1px solid #1D3350', borderRadius: 12, padding: 20,
    } as React.CSSProperties,
  }
  ...
  return (
    <div style={{ background: '#0C1825', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: 620, margin: '0 auto 80px', padding: '0 24px' }}>
        <div style={{ background: '#112236', border: '1px solid #1D3350', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #1D3350', display: 'flex', gap: 8 }}>
            {['#E05252','#D4943A','#4CAF82'].map((c,i) => <div key={i} style={{ width: 10, height: 10, borderRadius: '50%', background: c }}/>)}
            <span style={{ fontSize: 12, color: '#3A5570', marginLeft: 8 }}>amtsklar.at/analyse</span>
          </div>
          <div style={{ padding: 20 }}>
            <div style={{ background: 'rgba(224,82,82,0.1)', border: '1px solid rgba(224,82,82,0.25)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontSize: 10, color: '#4A6A87', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 4 }}>Ihre nächste Pflichtaktion</div>
            ...
            <div style={{ background: '#0C1825', borderRadius: 10, padding: '12px 16px' }}>
              <div style={{ fontSize: 10, color: '#4A6A87', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 6 }}>Was bedeutet dieser Brief?</div>
      ...
      {/* CTA */}
      <section style={{ textAlign: 'center', padding: '60px 24px 80px', borderTop: '1px solid #1D3350' }}>
        <h2 style={{ fontFamily: 'serif', fontSize: 28, fontWeight: 700, marginBottom: 12, color: '#EDF2FA' }}>Brief erhalten? Jetzt analysieren.</h2>
        <p style={{ color: '#6E8EAD', marginBottom: 28, fontSize: 15 }}>Erste Analyse kostenlos. Keine Kreditkarte nötig.</p>
      ...
      <footer style={{ borderTop: '1px solid #1D3350', padding: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 20, fontSize: 13 }}>
          {['Impressum','Datenschutz','Kontakt'].map(l => <a key={l} href={`/${l.toLowerCase()}`} style={{ color: '#3A5570', textDecoration: 'none' }}>{l}</a>)}
        </div>
      </footer>
    </div>
  )
