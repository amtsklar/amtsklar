...
export default function Landing() {
  ...
  const S = {
    btn: {
      ...
      color: '#EEF4FB', padding: '16px 32px', borderRadius: 12,
      fontSize: 17, fontWeight: 700, textDecoration: 'none',
    } as React.CSSProperties,
    card: {
      background: '#FFFFFF', border: '1px solid #C5D8ED', borderRadius: 12, padding: 20,
    } as React.CSSProperties,
  }
  ...
  return (
    <div style={{ background: '#EEF4FB', minHeight: '100vh' }}>
      ...
      {/* NAV */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid #C5D8ED', position: 'sticky', top: 0, background: '#FFFFFF', zIndex: 50, boxShadow: '0 1px 12px rgba(15,36,64,0.08)' }}>
        <Logo />
      ...
      <section style={{ textAlign: 'center', padding: '80px 24px 60px', maxWidth: 680, margin: '0 auto' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(201,150,58,0.12)', border: '1px solid rgba(201,150,58,0.25)', borderRadius: 20, padding: '6px 16px', fontSize: 13, color: '#C9963A', marginBottom: 24, fontWeight: 500 }}>
          🇦🇹 Speziell für Österreich · 45 Rechtsbereiche
        </div>
        <h1 style={{ fontFamily: 'Libre Baskerville,serif', fontSize: 'clamp(32px,6vw,52px)', fontWeight: 700, lineHeight: 1.15, marginBottom: 20, color: '#0F2440' }}>
          Brief vom Amt erhalten?<br />
          <span style={{ color: '#C9963A' }}>Jetzt sofort verstehen.</span>
        </h1>
        <p style={{ fontSize: 18, color: '#2A5080', lineHeight: 1.7, marginBottom: 36, maxWidth: 520, margin: '0 auto 36px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <a href="#preise" style={{ border: '1.5px solid #C5D8ED', color: '#2A5080', padding: '16px 24px', borderRadius: 12, fontSize: 15, textDecoration: 'none' }}>
            Preise ansehen
          </a>
        </div>
      ...
      <section style={{ maxWidth: 620, margin: '0 auto 80px', padding: '0 24px' }}>
        <div style={{ background: '#FFFFFF', border: '1px solid #C5D8ED', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #C5D8ED', display: 'flex', gap: 8 }}>
          ...
          <div style={{ padding: 20 }}>
            <div style={{ background: 'rgba(224,82,82,0.1)', border: '1px solid rgba(224,82,82,0.25)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontSize: 10, color: '#4A6A90', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 4 }}>Ihre nächste Pflichtaktion</div>
                  <div style={{ fontFamily: 'serif', fontSize: 17, fontWeight: 700, color: '#0F2440' }}>Einspruch schriftlich einbringen</div>
                  <div style={{ fontSize: 12, color: '#E05252', marginTop: 6 }}>🗓 Innerhalb 2 Wochen ab Zustellung</div>
                  <div style={{ fontSize: 13, color: '#1A3A5C', marginTop: 6, lineHeight: 1.5 }}>Brief an die Behörde: "Ich erhebe Einspruch gegen die Strafverfügung vom..." Per Einschreiben senden.</div>
                </div>
              </div>
            </div>
            <div style={{ background: '#F5F8FC', borderRadius: 10, padding: '12px 16px' }}>
              <div style={{ fontSize: 10, color: '#4A6A90', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 6 }}>Was bedeutet dieser Brief?</div>
              <div style={{ fontSize: 14, color: '#1A3A5C', lineHeight: 1.65 }}>Sie haben eine Strafverfügung erhalten. Der Magistrat fordert €180 Geldstrafe. Sie haben 2 Wochen Zeit um Einspruch zu erheben.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 900, margin: '0 auto 80px', padding: '0 24px' }}>
        <h2 style={{ textAlign: 'center', fontFamily: 'serif', fontSize: 30, fontWeight: 700, marginBottom: 40, color: '#0F2440' }}>Alles was du brauchst</h2>
      ...
      <section id="preise" style={{ maxWidth: 440, margin: '0 auto 80px', padding: '0 24px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'serif', fontSize: 30, fontWeight: 700, marginBottom: 8, color: '#0F2440' }}>Einfache Preise</h2>
        <p style={{ color: '#2A5080', marginBottom: 32, fontSize: 15 }}>1 kostenlose Analyse zum Testen</p>
        <div style={{ background: '#FFFFFF', border: '2px solid rgba(201,150,58,0.4)', borderRadius: 16, padding: '32px 28px' }}>
          <div style={{ fontFamily: 'serif', fontSize: 42, fontWeight: 700, color: '#0F2440', lineHeight: 1 }}>€2,99</div>
          <div style={{ color: '#2A5080', fontSize: 14, marginBottom: 24 }}>pro Monat · jederzeit kündbar</div>
          {['Unbegrenzte Analysen','Alle 45 Rechtsbereiche','Live RIS-Gesetzescheck','Fristen & Handlungsempfehlungen','Konsequenzen bei Nichttätigwerden','Kostenlose Beratungsstellen'].map((f,i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, textAlign: 'left', fontSize: 14, color: '#1A3A5C' }}>
          ...
          <div style={{ fontSize: 12, color: '#6A8AAA', marginTop: 12 }}>Erste Analyse kostenlos · Keine Kreditkarte nötig</div>
        </div>
      </section>

      {/* FAQ */}
      <section style={{ maxWidth: 640, margin: '0 auto 80px', padding: '0 24px' }}>
        <h2 style={{ fontFamily: 'serif', fontSize: 28, fontWeight: 700, marginBottom: 24, color: '#0F2440', textAlign: 'center' }}>Häufige Fragen</h2>
        {faqs.map((faq, i) => (
          <div key={i} style={{ borderBottom: '1px solid #C5D8ED', padding: '16px 0' }}>
            <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', background: 'none', border: 'none', color: '#0F2440', fontSize: 15, fontWeight: 600, cursor: 'pointer', textAlign: 'left', gap: 12 }}>
              {faq.q}<span style={{ color: '#C9963A', flexShrink: 0 }}>{openFaq === i ? '−' : '+'}</span>
            </button>
            {openFaq === i && <p style={{ marginTop: 10, fontSize: 14, color: '#2A5080', lineHeight: 1.65 }}>{faq.a}</p>}
          </div>
        ))}
      </section>
      ...
      <section style={{ textAlign: 'center', padding: '60px 24px 80px', borderTop: '1px solid #C5D8ED' }}>
        <h2 style={{ fontFamily: 'serif', fontSize: 28, fontWeight: 700, marginBottom: 12, color: '#0F2440' }}>Brief erhalten? Jetzt analysieren.</h2>
        <p style={{ color: '#2A5080', marginBottom: 28, fontSize: 15 }}>Erste Analyse kostenlos. Keine Kreditkarte nötig.</p>
        <Link to="/analyse" style={S.btn}>Kostenlos starten →</Link>
      </section>

      {/* FOOTER */}
      <footer style={{ borderTop: '1px solid #C5D8ED', padding: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ fontSize: 13, color: '#6A8AAA' }}>© 2025 AmtsKlar · Behördenbriefe sofort verstehen</div>
        <div style={{ display: 'flex', gap: 20, fontSize: 13 }}>
          {['Impressum','Datenschutz','Kontakt'].map(l => <a key={l} href={`/${l.toLowerCase()}`} style={{ color: '#6A8AAA', textDecoration: 'none' }}>{l}</a>)}
        </div>
      </footer>
    </div>
  )
