...
function isFreeLimitReached(count: number): boolean {
  ...
  if (localStorage.getItem(`ak_fp_${fp}`) === '1') return true
  return false
}
...
const SK = {
  ...
  email:       'ak_email',
  history:     'ak_history',
  savedResult: 'ak_saved_result',
  savedBrief:  'ak_saved_brief',
}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
              ...
              <div style={{ border: '1.5px solid #C5D8ED', borderRadius: 10, padding: '12px 8px', textAlign: 'center' }}>
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '7px 0', background: '#F5F8FC', border: '1.5px solid #C5D8ED', borderRadius: 7, fontSize: 11, fontWeight: 600, color: '#2A5080', cursor: 'pointer' }}
                >
                  {t.btn_choose}
                </button>
              </div>
              ...
              <div style={{ border: '2px solid #C9963A', borderRadius: 10, padding: '12px 8px', textAlign: 'center', background: 'rgba(201,150,58,0.04)', position: 'relative' }}>
                <div style={{ position: 'absolute', top: -9, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg,#B8832A,#D4A84B)', color: '#FFFFFF', fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                  {t.badge_popular}
                </div>
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '7px 0', background: 'linear-gradient(135deg,#B8832A,#D4A84B)', border: 'none', borderRadius: 7, fontSize: 11, fontWeight: 700, color: '#FFFFFF', cursor: 'pointer' }}
                >
                  {t.btn_choose_arrow}
                </button>
              </div>
            ...
            <div style={{ borderTop: '1px solid #C5D8ED', paddingTop: 12, marginTop: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button
                  ...
                  disabled={verifying}
                  style={{ padding: '8px 12px', borderRadius: 7, fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', flexShrink: 0, background: '#F5F8FC', border: '1.5px solid #C5D8ED', color: '#2A5080', cursor: 'pointer' }}
                >
      ...
      {showLongTextWarning && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,36,64,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #C5D8ED', borderRadius: 16, padding: '28px 24px', maxWidth: 480, width: '100%', boxShadow: '0 20px 60px rgba(15,36,64,0.2)' }}>
            ...
            <div style={{ background: 'rgba(201,150,58,0.08)', border: '1px solid rgba(201,150,58,0.25)', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: '#8B6020', lineHeight: 1.65 }}>
              {t.longtext_tip}
            </div>
            <div style={{ display: 'flex', gap: 10, flexDirection: 'column' }}>
              <button onClick={() => analyse(true)} style={{ padding: '13px', background: 'linear-gradient(135deg,#B8832A,#D4A84B)', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, color: '#FFFFFF', cursor: 'pointer' }}>
                {t.longtext_btn_analyse}
              </button>
              <button onClick={() => setShowLongTextWarning(false)} style={{ padding: '13px', background: '#F5F8FC', border: '1.5px solid #C5D8ED', borderRadius: 10, fontSize: 14, fontWeight: 600, color: '#2A5080', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {isPaid ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: '#4CAF82', background: 'rgba(76,175,130,0.1)', border: '1px solid rgba(76,175,130,0.3)', borderRadius: 6, padding: '3px 10px' }}>
                ✓ {planLabel[plan]}
              </span>
              <a
                ...
                style={{ fontSize: 11, color: '#6A8AAA', textDecoration: 'none', border: '1px solid #C5D8ED', borderRadius: 6, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}
              >
        ...
        <div style={S.inner}>
          ...
          {screen === 'input' && !loading && (
            <div className="fade-in">
              <h1 style={{ fontFamily: 'Libre Baskerville,serif', fontSize: 28, fontWeight: 700, lineHeight: 1.2, color: '#0F2440', marginBottom: 8 }}>
                Brief<br/>erhalten?
              </h1>
              ...
              <div
                ...
                onDrop={handleDrop}
                style={{
                  border: `2px dashed ${isDragOver ? '#C9963A' : imageData ? '#4CAF82' : pdfFileName ? '#4CAF82' : '#C5D8ED'}`,
                  borderRadius: 12,
                  padding: imageData ? '12px 20px' : '20px',
                  marginBottom: 12,
                  background: isDragOver ? 'rgba(201,150,58,0.06)' : (imageData || pdfFileName) ? 'rgba(76,175,130,0.05)' : '#FAFCFF',
                  textAlign: 'center',
              ...
              >
                ...
                {(pdfLoading || imageLoading) ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); setImageData(null) }} style={{ background: 'none', border: 'none', color: '#6A8AAA', cursor: 'pointer', fontSize: 20, flexShrink: 0 }} title="Entfernen">×</button>
                  </div>
                ) : pdfFileName ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); setPdfFileName(null); setBriefText('') }} style={{ marginLeft: 8, background: 'none', border: 'none', color: '#6A8AAA', cursor: 'pointer', fontSize: 18 }} title="Entfernen">×</button>
                  </div>
                ) : isDragOver ? (
                  <div>
                    <div style={{ fontSize: 28, marginBottom: 6 }}>📂</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#C9963A' }}>{t.drop_over}</div>
                  </div>
                ) : (
                  <div>
              ...
              {!imageData && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#6A8AAA', whiteSpace: 'nowrap' }}>{t.text_divider}</span>
                  <div style={{ flex: 1, height: 1, background: '#C5D8ED' }}/>
                </div>
              )}
              ...
              {!imageData && (
                <textarea
                  style={{ ...S.ta, border: pdfFileName ? '1.5px solid rgba(76,175,130,0.5)' : '1.5px solid #C5D8ED' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, marginBottom: 16 }}>
                  <div style={{ fontSize: 12 }}>
                    {briefText.length > CHAR_DANGER ? (
                      <span style={{ color: '#E05252', fontWeight: 600 }}>{t.char_long}</span>
                    ) : briefText.length > CHAR_WARN ? (
                      <span style={{ color: '#D4943A', fontWeight: 500 }}>{t.char_warn}</span>
                    ) : (
              ...
              {error && (
                <div style={{ background: 'rgba(224,82,82,0.08)', border: '1px solid rgba(224,82,82,0.25)', borderRadius: 10, padding: '12px 16px', color: '#E05252', fontSize: 14, marginBottom: 14 }}>
                  ⚠️ {error}
                </div>
              )}

              <button
                style={{ ...S.btn, opacity: (imageData !== null || briefText.trim().length >= 20) ? 1 : 0.5, cursor: (imageData !== null || briefText.trim().length >= 20) ? 'pointer' : 'not-allowed' }}
          ...
          {screen === 'result' && result && !loading && (
            <div className="fade-in">
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
                <div>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(201,150,58,0.12)', border: '1px solid rgba(201,150,58,0.28)', borderRadius: 8, padding: '5px 12px', fontSize: 13, color: '#C9963A', fontWeight: 500, marginBottom: 6 }}>
                    📄 {result.brieftyp}
                  </div>
                  <div style={{ fontSize: 12, color: '#4A6A90' }}>Von: {result.behoerde}</div>
                </div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, borderRadius: 8, padding: '6px 12px', fontSize: 13, fontWeight: 600, background: urg.bg, border: `1px solid ${urg.border}`, color: urg.color }}>
                ...
                <div style={{ borderRadius: 14, padding: 20, marginBottom: 14, border: `2px solid ${urg.border}`, background: urg.bg }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 14 }}>
                    <span style={{ fontSize: 24, flexShrink: 0 }}>
                      {result.handlungsempfehlung.prioritaet === 'kritisch' ? '🚨' : result.handlungsempfehlung.prioritaet === 'hoch' ? '⚡' : '📌'}
                    </span>
                    <div>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.6)', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {result.handlungsempfehlung.wie && (
                      <div style={{ fontSize: 14, lineHeight: 1.65, color: '#1A3A5C', borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: 10 }}>
                        {result.handlungsempfehlung.wie}
                      </div>
                    )}
                  </div>
                ...
                <div style={S.card}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                    {result.rechtsgrundlage.map((r, i) => (
                      <a key={i} href={getRISUrl(r)} target="_blank" rel="noopener noreferrer" style={{ background: '#F5F8FC', border: '1px solid #C5D8ED', borderRadius: 6, padding: '4px 10px', fontSize: 12, color: '#2A5080', fontFamily: 'monospace', textDecoration: 'none' }}>
                        {r} ↗
                      </a>
                    ))}
                  </div>
              ...
              {planHatBrief(plan) && antwortbrief ? (
                <div style={{ background: '#FFFFFF', border: '2px solid rgba(201,150,58,0.4)', borderRadius: 14, padding: 20, marginBottom: 12, boxShadow: '0 4px 20px rgba(201,150,58,0.1)' }}>
                  ...
                  <div style={{ background: 'rgba(201,150,58,0.08)', border: '1px solid rgba(201,150,58,0.2)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#C9963A', lineHeight: 1.6 }}>
                    💡 {antwortbrief.hinweis}
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <button onClick={copyBrief} style={{ padding: '11px', border: '1.5px solid #C5D8ED', background: copied ? 'rgba(76,175,130,0.1)' : '#F5F8FC', borderColor: copied ? 'rgba(76,175,130,0.4)' : '#C5D8ED', borderRadius: 10, fontSize: 14, fontWeight: 600, color: copied ? '#4CAF82' : '#2A5080', cursor: 'pointer' }}>
                      {copied ? t.btn_copied : t.btn_copy}
                    </button>
                    <button onClick={downloadBrief} style={{ padding: '11px', background: 'linear-gradient(135deg,#B8832A,#D4A84B)', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, color: '#FFFFFF', cursor: 'pointer' }}>
                      {t.btn_dl}
                    </button>
                  </div>
                </div>
                ...
                <div style={{ background: 'rgba(201,150,58,0.06)', border: '1.5px solid rgba(201,150,58,0.25)', borderRadius: 14, padding: 20, marginBottom: 12, textAlign: 'center' }}>
                  ...
                  <button onClick={() => setShowPaywall(true)} style={{ background: 'linear-gradient(135deg,#B8832A,#D4A84B)', color: '#FFFFFF', border: 'none', borderRadius: 10, padding: '12px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                    {t.upgrade_btn}
                  </button>
                </div>
              ) : null}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: 'Libre Baskerville,serif', fontSize: 15, fontWeight: 700, color: '#0F2440', marginBottom: 4 }}>
                        {lang === 'de' ? 'Ergebnis per E-Mail + Frist-Erinnerung?' : lang === 'en' ? 'Get results by email + deadline reminder?' : lang === 'tr' ? 'Sonuç e-posta + son tarih hatırlatması?' : lang === 'it' ? 'Risultato via e-mail + promemoria scadenza?' : lang === 'ru' ? 'Результат по e-mail + напоминание о сроке?' : lang === 'pl' ? 'Wynik e-mailem + przypomnienie o terminie?' : lang === 'ro' ? 'Rezultat pe e-mail + reminder termen?' : lang === 'hu' ? 'Eredmény e-mailben + határidő emlékeztető?' : lang === 'sr' ? 'Rezultat e-mailom + podsetnik roka?' : lang === 'hr' ? 'Rezultat e-mailom + podsjetnik roka?' : lang === 'sl' ? 'Rezultat po e-pošti + opomnik roka?' : lang === 'sk' ? 'Výsledok e-mailom + pripomienka termínu?' : 'Get results by email + deadline reminder?'}
                      </div>
                      <div style={{ fontSize: 13, color: '#2A5080', marginBottom: 12, lineHeight: 1.5 }}>
                        {lang === 'de' ? 'Wir senden dir das Ergebnis und erinnern dich rechtzeitig an deine Frist. Kostenlos, kein Spam.' : lang === 'en' ? "We'll send you the analysis and remind you before your deadline. Free, no spam." : lang === 'tr' ? 'Analiz sonucunu ve son tarihi hatırlatacağız. Ücretsiz, spam yok.' : lang === 'it' ? "Ti inviamo l'analisi e ti ricordiamo prima della scadenza. Gratis, niente spam." : lang === 'ru' ? 'Пришлём результат и напомним о сроке. Бесплатно, без спама.' : lang === 'pl' ? 'Wyślemy wynik i przypomnimy o terminie. Bezpłatnie, bez spamu.' : lang === 'ro' ? 'Trimitem rezultatul și vă amintim termenul. Gratuit, fără spam.' : lang === 'hu' ? 'Elküldjük az eredményt és emlékeztetünk a határidőre. Ingyen, spam nélkül.' : "We'll send you the analysis and remind you before your deadline. Free, no spam."}
                      </div>
              ...
              <div style={{ background: '#F5F8FC', border: '1px solid #C5D8ED', borderRadius: 12, padding: '14px 16px', marginTop: 20, marginBottom: 4, fontSize: 11, color: '#6A8AAA', lineHeight: 1.65, textAlign: 'center' }}>
                ...
                {t.legal_text}{' '}
                Quelle Gesetze:{' '}
                <a href="https://www.ris.bka.gv.at" target="_blank" rel="noopener noreferrer" style={{ color: '#2A5080' }}>RIS.bka.gv.at</a>
                {' '}(CC BY 4.0)
              </div>
