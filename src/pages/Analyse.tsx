...
export default function Analyse() {
  ...
  const openCheckout = () => {
    ...
    if (token && (window as any).Paddle) {
      const priceId = (import.meta as any).env?.VITE_PADDLE_PRICE_VERSTEHEN
      ...
      alert('Zahlung wird in Kürze verfügbar. Bitte probiere es später nochmal.')
    }
  }
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#112236', border: '1.5px solid rgba(201,150,58,0.4)', borderRadius: 16, padding: '32px 28px', maxWidth: 420, width: '100%' }}>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 14, color: '#6E8EAD', lineHeight: 1.6 }}>Deine kostenlose Analyse ist aufgebraucht.<br/>Unbegrenzte Analysen ab €2,99/Monat.</div>
            </div>
            <button onClick={openCheckout} style={{ ...S.btn, marginBottom: 16 }}>Jetzt abonnieren →</button>
        ...
        <div style={S.inner}>
          ...
          {screen === 'input' && !loading && (
            <div>
              ...
              <textarea style={S.ta} placeholder={'Brieftext hier einfügen…\n\nz.B. Strafverfügung, Finanzamtsbescheid, AMS-Schreiben, Inkasso, Mitkündigung…'}
          ...
          {screen === 'result' && result && !loading && (
            <div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid #192B42', borderRadius: 12, padding: '14px 16px', marginTop: 20, marginBottom: 4, fontSize: 11, color: '#3A5570', lineHeight: 1.65, textAlign: 'center' }}>
                ⚖️ <strong style={{ color: '#4A7A9A' }}>Rechtlicher Hinweis:</strong> AmtsKlar dient zur Information. Ersetzt keine Rechtsberatung.{' '}
                Quelle: <a href="https://www.ris.bka.gv.at" target="_blank" rel="noopener noreferrer" style={{ color: '#4A7A9A' }}>RIS.bka.gv.at</a> (CC BY 4.0)
              </div>
